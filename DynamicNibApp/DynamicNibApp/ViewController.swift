...
class ViewController: UIViewController {
    ...
    @IBAction func openDynamicPage(_ sender: UIButton) {
        let dynamicNibURL = URL(string: "http://127.0.0.1:8080/nibs/\(DynamicViewController.self)")!
        URLSession.shared.downloadTask(with: dynamicNibURL) { localURL, response, error in
            ...
            do {
                ...
                let nibURL = bundle.bundleURL.appending(path: "\(DynamicViewController.self).nib")
                try manager.moveOrReplaceItem(at: localURL, to: nibURL)
                DispatchQueue.main.async {
                    let controller = DynamicViewController(nibName: "\(DynamicViewController.self)", bundle: bundle)
